module ncdump
 ...
 subroutine ncsavebike(x,y,thck,topg,beta, magvel, velc, &
      divuh, usrf, temp,n,m,upn,file)
    ...
    real(kind=8), dimension(1:n,1:m) ,intent(in):: thck, topg, beta, magvel,&
         velc, divuh, usrf
    ...
    call nccheck( nf90_def_var(nc_id, "thck", nf90_real8, cell_dim_id, var_id) )
    ...
    call nccheck( nf90_def_var(nc_id, "magvel", nf90_real8, cell_dim_id, var_id) )
    ...
    call nccheck( nf90_def_var(nc_id, "usrf", nf90_real8, cell_dim_id, var_id) )
    ...
    !cell centred arrays
    call nccheck( nf90_inq_varid(nc_id, "thck", var_id) )
    ...
    call nccheck( nf90_inq_varid(nc_id, "magvel", var_id) )
    call nccheck( nf90_put_var(nc_id, var_id , magvel) )
    ...
    call nccheck( nf90_inq_varid(nc_id, "usrf", var_id) )
    call nccheck( nf90_put_var(nc_id, var_id , usrf) )
...
! don't need to call yflip for GIS dataset, unless you're an Aussie
!  call yflip(a,ewn,nsn)
end subroutine readone

subroutine readdata(topg,thck,magvel,ewn,nsn)
 integer, parameter :: ewnin = 301, nsnin = 561, ewoff = 5, nsoff = 1
 integer :: ewn,nsn
 real(kind=8), dimension(1:ewn,1:nsn) :: usrf,lsrf,topg,thck,magvel
 character(len=12) :: file

 !file = 'topg_5gis.dat'
 call readone(topg,'topg_5gis.dat',ewn,nsn,ewnin,nsnin,ewoff,nsoff)
 !file = 'thck_5gis.dat'
 call readone(thck,'thck_5gis.dat',ewn,nsn,ewnin,nsnin,ewoff,nsoff)
 !file = 'balvelmag_5gis.dat'
 call readone(magvel,'balvelmag_5gis.dat',ewn,nsn,ewnin,nsnin,ewoff,nsoff)
...
program t
  ...
  integer, parameter :: ewn = 288, nsn = 560, upn = 10
  
  real(kind=8),parameter :: scyr = 31556926.0d0, rhoi = 918.0d0, rhoo = 1028.0d0,  &
       grav = 9.81d0, dx = 5.0d+3, dy = dx,  tol = 1.0d-3, minbeta = 2.0, &
       maxbeta = 1.0e+6, seabeta = 100.0, shelfbeta = 1.0e-5,cm = 1.0e-2, &
       glen_n = 3.0, maxseabeta = 100.0, lambda = 4.0e+3
  
  real (kind = 8), dimension(1:ewn,1:nsn) :: topg, lsrf, usrf, thck, magvel, velc,divuh, &
  ...
  call readdata(topg,thck,magvel,ewn,nsn)
...
!!$
  ...
  filename =  'Greenland-bisicles-5km.nc'
  call ncsavebike(x,y,thck,topg,beta,magvel,velc,divuh,usrf,temp,ewn,nsn,upn,filename)
