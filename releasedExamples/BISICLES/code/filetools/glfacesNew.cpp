...
//===========================================================================
// glfacesNew.cpp
// A modified version of glfaces.cpp. Reads in a bisicles plotfile and dump out a list of x,y coordinates 
// which sit on the grounding line. Also creates a new bisicles plotfile with 3 variables - 
// sftgrf, sftflf, and ligroundf. 
//===========================================================================
...
{ // Begin nested scope
  ...
  if(argc != 4) 
    { 
      std::cerr << " usage: " << argv[0] << " <plot_file> <config_file> <output_file>" << std::endl; 
      exit(0); 
    }
  ...
  if (problem_type == "basic")
  ...
  else if (problem_type == "Python")
    {
      ...
      PythonInterface::PythonIBC* ptr = new PythonInterface::PythonIBC(module, thckFuncName, topgFuncName, rhsFuncName,faceVelFuncName);
      thicknessIBC = static_cast<IceThicknessIBC*>( ptr);
    }
#endif
  else 
  ...
  for (int lev = 0; lev < numLevels; lev++)
    {
      ...
      newData[lev] = new LevelData<FArrayBox>(grids[lev], newDataNumComps, IntVect::Zero);
              ...
              for (BoxIterator bit(b);bit.ok();++bit)
                {
                  ...
                  thisNewData(iv,3) = thisDragCoef(iv,0)*sqrt((u(iv,0)*u(iv,0)) + u(iv,1)*u(iv,1));
                  
                  
                  //Set the floating fraction value at ivp to be 1.0 if the ice is floating
                  if (validMask(iv) == 1 && floatingMask(iv) == FLOATINGMASKVAL)
                    {
                      thisNewData(iv, 1) = 1.0;
                    }
                  else if (validMask(iv) == 1 && floatingMask(iv) == GROUNDEDMASKVAL)
                    {
                      // component 0 is the grounded fraction
                      // component 1 is the floating fraction
                      // component 2 is the GL flux
                      
                      //set the grounded fraction value at iv to be 1.0 if the ice is grounded
                      thisNewData(iv, 0) = 1.0;
                          ...
                          if ((floatingMask(ivp) == FLOATINGMASKVAL) || (floatingMask(ivp) == OPENSEAMASKVAL))
                            {
                              ...
                              //face centered point between iv and ivp
                              ...
                              //grid index on face centered grids
                              for (int dir = 0; dir < SpaceDim; dir++)
                                {
                                  pout() << ivf[dir] << " ";
                                }
                              ...
                              for (int dir = 0; dir < SpaceDim; dir++)
                                {
                                  fvel[dir] = 0.5*(u(iv,dir)+u(ivp,dir));
                                  pout() << fvel[dir] << " ";
                                }
                              
                              thisNewData(iv, 2) += max((abs(fvel[fdir]) * flotationThicknessFactor * ftopg), 0.0);
                            ...
                            } //closing bracket for floating/opensea check
                        } //end loop for (left or right)(fdir = 0) or (up or down)(fdir = 1)
                    } //closing bracket for grounded check
                } //end loop over cells
  ...
  Vector<std::string> newNames(newDataNumComps);
  newNames[0] = "sftgrf"; //grounded ice sheet area fraction
  newNames[1] = "sftflf"; //floating ice sheet area fraction
  newNames[2] = "ligroundf"; //grounding line flux
  newNames[3] = "strbasemag"; // basal drag
