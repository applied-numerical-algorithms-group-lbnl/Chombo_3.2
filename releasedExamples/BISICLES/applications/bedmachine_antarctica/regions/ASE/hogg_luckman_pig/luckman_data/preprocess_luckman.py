...
def open_luckman_100m(file):  
    ...
    Nx, Ny = len(xt)+4,len(yt)+4
    dx,dy = xt[1]-xt[0], yt[1]-yt[0]
    ox,oy = np.min(xt) - 2.0*dx, np.min(yt) - 2.0*dy
    xa = np.arange(ox,ox+dx*Nx-eps,dx)
    ya = np.arange(oy,oy+dy*Ny-eps,dy)
    
    umod = np.zeros( (Ny,Nx) )    
    umod[2:Ny-2,2:Nx-2] = nc.variables['z'][:,:]*365.24
...
def open_luckman_year(year, extra_mask=None):
    import glob
#   mean_mosaic.201410_201412.6or12-day.1to2-pairs.tracks-065.mosaics-7.track_filt.mean.mag.nc
    f = 'speeds/mean_mosaic.20{}??_20{}??.*.nc'
    print (f)
    ...
    xa,ya,umoda,maska = open_luckman_100m(f[0])
    ...
    for ff in f[1:]:
        xb,yb,umodb,maskb = open_luckman_100m(ff)
    ...
    mf = '../hogg_data/masks/pinei_20{}.tif'.format(year)
...
def makehdf5(year,extra_mask=None):
    xa,ya,umoda,maska =   open_luckman_year(year,extra_mask)
    ncf = 'luckman_pig_{}.nc'.format(year)
    save_nc(x,y,{ 'uo' : umoda , 'uc' : maska}, ncf)

    nctoamr = '~/Development/BISICLES/code/filetools/nctoamr2d.Linux.64.g++.gfortran.DEBUG.OPT.ex'
    hdf5 = 'luckman_pig_{}.2d.hdf5'.format(year)
...
nc15 = Dataset('luckman_pig_19.nc','r') # best coverage
ucm =  nc15.variables['uc'][:,:]
ncf= 'luckman_pig_14.nc'
uo = np.where(ucm > 0.1, uo, 0.0)
save_nc(x,y,{ 'uo' : uo , 'uc' : ucm}, ncf)
hdf5 = 'luckman_pig_14.2d.hdf5'
nctoamr = '~/Development/BISICLES/code/filetools/nctoamr2d.Linux.64.g++.gfortran.DEBUG.OPT.ex'
cmd = nctoamr + ' ' + ncf + ' ' + hdf5 + ' uo uc'    
os.system(cmd)  