    ...
    for (levelDit.begin(); levelDit.ok(); ++levelDit)
    {
      ...
      if (s_probtype == exact)
      {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
          ...
          thisRhs(iv, 0) = ( 2*plus_arg + thisU(iv, 0));
      ...
      else if (s_probtype == inexact)
      {
        ...
        for (bit.begin(); bit.ok(); ++bit)
        {
          ...
          D_TERM(x = loc[0];, y = loc[1];,  z = loc[2];)
            thisRhs(iv, 0) = ( 9*M_PI*M_PI*(x*x-x*x*x) + 6*x - 2 )*sin( 3*M_PI*y ) + thisU(iv, 0);
  ...
  if (maxLevel > 0)
  {
    ...
    else
    {
      ...
      while (moreLevels)
      {
        ...
        setRHS(tempRHS, tempNL, a_amrDomains, a_refRatios, a_amrDx,
               a_finestLevel);
            ...
            int a_finestLevel)
{
  ...
  if (nonlinearOp) {
      ...
      // NonLinear portion
      ExternalObj a_extObj = ExternalObj();
      NL_level NLfunctTmp  = &ExternalObj::NonLinear_level;
...
int runSolver()
{
  ...
  // Non Linear Part
  Vector<LevelData<FArrayBox>* > NLfunc(numLevels, NULL);
  Vector<LevelData<FArrayBox>* > NLdfunc(numLevels, NULL);
  ...
  ExternalObj a_extObj = ExternalObj();
  a_extObj.NonLinear_piece(NLfunc, NLdfunc, exact, finestLevel);
  setRHS(rhs, NLfunc, amrDomains, refRatios, amrDx, finestLevel );
