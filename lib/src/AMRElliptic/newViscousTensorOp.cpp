...
ViscousTensorOpFactory(const Vector<DisjointBoxLayout>&                     a_grids,
                       ...
                       int                                                  a_relaxMinIter)
{
  CH_TIME("oldVTOFactory constructor1");
  setDefaults();
  BCHolder bc(a_bc);
...
{
  CH_TIME("oldVTOFactory constructor2");
  setDefaults();
  define(a_grids,a_eta,a_lambda,a_acoef,a_alpha,a_beta,
...
{
  CH_TIME("oldVTOFactory::AMRnewOp");
  int ref = 0;
  bool found = false;
  for (int ivec = 0; ivec < m_domains.size(); ivec++)
  {
...
{
  CH_TIME("oldVTO::homogeneousCFinterp");
